...
namespace Biscuit {
	...
	xImageHelper::eFREE_IMAGE_FORMAT xImageHelper::GetImageFileType(String^ filename) {
		pin_ptr<const wchar_t> wch = PtrToStringChars(filename);

		std::filesystem::path path(wch);
		std::error_code ec;
		if (!std::filesystem::is_regular_file(path, ec))
			return eFREE_IMAGE_FORMAT::FIF_UNKNOWN;
		size_t sizeFile = std::filesystem::file_size(path, ec);
		if (!sizeFile)
			return eFREE_IMAGE_FORMAT::FIF_UNKNOWN;
		...
		auto strExt = path.extension().string();
		auto eFileType = FreeImage_GetFIFFromFilename(strExt.c_str());
		return (eFREE_IMAGE_FORMAT)eFileType;
	}

	bool xImageHelper::IsImageFile(String^ filename) {
		return GetImageFileType(filename) != eFREE_IMAGE_FORMAT::FIF_UNKNOWN;
	}
	...
	bool xImageHelper::PackValue(CV::Mat^ mat, uint32_t row0, uint32_t row1, int32_t bpp, array<int>^ buffer, int^ pos) {
